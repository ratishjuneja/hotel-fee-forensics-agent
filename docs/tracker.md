...
| Vultr inference | Person A | ✅ In live path | OpenAI-compatible client (PR #3); 30s timeout + `max_tokens` cap + https enforcement (PR #16); PR-10 wires `chatComplete` (temp 0, one transient retry) into `runAudit` as the injected LLM boundary — unconfigured env → loud 503, never a fake audit; PR-11 adds dual-model routing (retrieval steps → `VULTR_INFERENCE_RETRIEVER_MODEL`; extraction/prose stay on the default model); **live smoke PASSED 2026-07-04** — 3 consecutive golden runs ($36,580 / 96%, zero warnings) on `deepseek-ai/DeepSeek-V4-Flash` after PR-12 hardening; note VultronRetriever* ids turned out not to serve `/chat/completions` (they back the managed vector store), and Qwen3.5/3.6/Nemotron are reasoning models (thinking tokens vs the 1,500-token cap); live run latency 30–70s |
| Retrieval | Person A | ✅ Wired | `retriever.ts` — model-driven chunk selection on a VultronRetriever model (injected boundary, tested); wired into the orchestrator (steps 2, 3, 7); live Vultr call wired in PR-10; runs on the dedicated VultronRetriever model when `VULTR_INFERENCE_RETRIEVER_MODEL` is set (PR-11) |
