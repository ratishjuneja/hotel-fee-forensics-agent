...
| Vultr inference | Person A | ✅ In live path | OpenAI-compatible client (PR #3); 30s timeout + `max_tokens` cap + https enforcement (PR #16); PR-10 wires `chatComplete` (temp 0, one transient retry) into `runAudit` as the injected LLM boundary — unconfigured env → loud 503, never a fake audit; **live smoke PASSED 2026-07-04** — 3 consecutive golden runs ($36,580 / 96%, zero warnings) after PR-12 hardening, re-validated ×3 after PR-13; **primary workflow = VultronRetrieverPrime via `/v1/rerank`** (hackathon requirement), secondary generation = `deepseek-ai/DeepSeek-V4-Flash` chat (only catalog chat model without reasoning-token overhead; Qwen3.5/3.6/Nemotron think against the 1,500-token cap); live run latency ~28–70s |
...
- [x] Put VultronRetriever in the primary workflow (PR-13: the sponsor's models are retrieval scorers, not chat models — `/chat/completions` 404s for them and the HF cards say visual-document-retrieval — so all three retrieval steps now score chunks on `vultr/VultronRetrieverPrime-Qwen3.5-8B` via Vultr's `/v1/rerank` (`rankRelevantChunks` in `retriever.ts`, injected `ChunkRanker` boundary, compound queries split per intent for top-1 precision); generation (clause transcription + prose) stays on a secondary chat model as the rules permit; `buildServer({ llm, ranker })` keeps tests transport-free, and an injected test llm is never silently paired with a live ranker; obsolete chat-routing (`pickModel`, PR-11) removed; 3 consecutive golden live runs with the chat model banned from retrieval in the route test)
